...
def tab_join_create_hole_on_plane(tab_face, width, pos_y, material_face, material_plane, dog_bone=False):
    ...
    # Extended: force the boolean tool to cross both limiting faces.
    hole_through_margin = max(
        0.0,
        float(getattr(tab_face, "cut_through_margin", 0.10))
    )
    corrected_length = material_plane.thickness + 2.0 * hole_through_margin
    ...
    origin = FreeCAD.Vector(
        -hole_through_margin,
        -corrected_width_center,
        -corrected_height_center
    )
